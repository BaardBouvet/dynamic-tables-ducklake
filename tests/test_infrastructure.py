...
def test_metadata_store_init(metadata_store):
...
def test_can_connect_to_duckdb(duckdb_conn):
...
def test_duckdb_s3_integration(duckdb_conn, minio_client):
...
@pytest.mark.skip(reason="DuckLake snapshot time-travel needs investigation - may require specific configuration")
def test_ducklake_snapshots(duckdb_conn):
