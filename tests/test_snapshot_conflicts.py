    ...
    def test_conflicting_snapshots_from_multiple_dependencies(
        ...
        refresher.create_dynamic_table(
            DynamicTableDefinition.create(
                name="order_summary_b",
                schema_name="main",
                query_sql="SELECT product_id, SUM(amount) as total FROM orders GROUP BY product_id",
            )
        )
        ...
        refresher.create_dynamic_table(
            DynamicTableDefinition.create(
                name="order_summary_c",
                schema_name="main",
                query_sql="SELECT product_id, COUNT(*) as count FROM orders GROUP BY product_id",
            )
        )
